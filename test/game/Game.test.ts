...
describe("Game", async () => {
  ...
  describe("Withdraw token", () => {
    ...
    it("returns right data - native token", async () => {
      const tx1 = await GameContract.connect(user).depositNativeToken(
  ...
  describe("Deposit token", () => {
    context("when deposit RUNNOW token", async () => {
      context("when success", async () => {
        ...
        it("updates right balance of user and balance of game", async () => {
          ...
          const balanceOfUserAfter = await RUNNOWContract.connect(
            user
          ).balanceOf(user.address);
          const balanceOfGameAfter = await RUNNOWContract.connect(
            user
          ).balanceOf(GameContract.address);

          expect(balanceOfUserAfter.toString()).to.equal(
            ethers.utils.parseEther("700")
          );
          expect(balanceOfGameAfter.toString()).to.equal(
            ethers.utils.parseEther("300")
          );
        });
      });
      ...
      context("when error", async () => {
        context("when user balance equal 0", async () => {
          it("returns transfer amount exceeds balance", async () => {
            ...
            await expect(
              GameContract.connect(user).depositToken(
                ethers.utils.parseEther("300"),
                RUNNOWContract.address
              )
    ...
    context("when deposite RUNGEM token", async () => {
      context("when success", async () => {
        ...
        it("updates right balance of user and balance of game", async () => {
          ...
          const balanceOfUserAfter = await RUNGEMContract.connect(
            user
          ).balanceOf(user.address);
          const balanceOfGameAfter = await RUNGEMContract.connect(
            user
          ).balanceOf(GameContract.address);
    ...
    context("when deposit NATIVE token", async () => {
      context("when success", async () => {
        it("updates right balance of user and balance of game", async () => {
          const balanceOfUserBefore = await provider.getBalance(user.address);
          let tx = await GameContract.connect(user).depositNativeToken(
            ethers.utils.parseEther("5"),
            {
              value: ethers.utils.parseEther("5"),
            }
  ...
  it("Deposit item", async () => {
    // Upgrade
    const NFTContractV2 = await upgradeNFTUpgradeable(
      NFTContract.address,
      deployer
    );
